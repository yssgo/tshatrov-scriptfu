...
(define (edit-paste-into-one-drawable drawable paste-into)
  ;; returns integer : list of one item, which is the ID of the first item in the list of layers or floating-selection
  (let* ((new-drawables (car (gimp-edit-paste drawable paste-into))))
    (list (vector-ref new-drawables 0))))

(define (floating-sel-check-and-anchor floating-sel)
  (let* ((is-floating (car (gimp-layer-is-floating-sel floating-sel))))
    (if (py-True? is-floating)
...
(define (script-fu-flatten-layer-groups-filter InImage InDrawables)
  (flatten-layer-groups InImage))

(script-fu-register-filter
 "script-fu-flatten-layer-groups-filter"
...
(script-fu-menu-register "script-fu-flatten-layer-groups-filter"
                         ;; FOR TRANSLATORS: Don't translate '<Image>/Image/'
                         _"<Image>/Image/Timofei Shatrov"
                         )
(script-fu-register-i18n "script-fu-flatten-layer-groups-filter" "Standard")
...
(define (get-roll-layer-list layer)
  (if (py-True? (car (gimp-item-is-group layer)))
...
(define (layer-matte-cutout img layer bg-layer threshold)
  ...
  (if (py-True? (car (gimp-selection-bounds img)))
      (set! sel (car (gimp-selection-save img))))
...
(define (animstack-get-interval group only interval)
  (if (py-True? (car (gimp-item-is-group group)))
...
(define (animstack-sample-to-target img source target x y width height)
  ...
  (if (py-True? (car (gimp-edit-copy (vector source))))
      (let ((fl (car (edit-paste-into-one-drawable target FALSE)))
        ...
        (floating-sel-check-and-anchor fl)))
  (animstack-restore-selection img))
         ...
         (let* ((tags (sort-animstack-tags (extract-animstack-tags layer)))
           ...
           (if (py-True? (car (gimp-item-is-group layer)))
               (cons (list before-effects during-effects)
                     (build-dup-branch (vector->list (car (gimp-item-get-children layer)))))
  ...
  (with-params
   ...
   (let* ((group (groupify-layer img layer))
     ...
     (if (= (length contents) 0)
         (begin
           (gimp-message* "Empty duplicate tree !")
...
(define (animstack-process-tag img layer tag generator-alist before-effects during-effects extra-opts)
  (let* ((tagpair '())
         ...
         (tag-assoc '())
         (ret-vals '()))
    (set! tagpair (animstack-parse-tagname (car tag))) ;; (tagname . other opts)
    (set! tagname (car tagpair))
    (set! opts (apply list (cdr tagpair) generator-alist before-effects during-effects extra-opts))
    (set! tag-assoc (*animstack-action-tag-assocs* 'assoc tagname))
...
(define (animstack-erase img params)
  ...
  (with-params
   ...
   (cons (lambda (layer target)
           (if (py-True? (car (gimp-item-id-is-text-layer layer)))
...
(define (animstack-add-margin layer margin)
  (cond ((py-True? (car (gimp-item-is-group layer)))
         (vector-for-each
...
(define (autocrop-layer-transparent img layer margin)
  ...
  (if (py-False? (car (gimp-item-is-group layer)))
...
(define (linear_motion_blur layer len angle)
  ...
  (begin
    ...
    (let* ((layer_neww (car (gimp-drawable-get-width layer)))
           (layer_newh (car (gimp-drawable-get-height layer)))
           (layer_newx (car (gimp-drawable-get-offsets layer)))
           (layer_newy (cadr (gimp-drawable-get-offsets layer)))
           (layer_oldx '())
           (layer_oldy '())
           (mblurlinear '()))
      (set! mblurlinear
            (car (gimp-drawable-filter-new layer
                                           "gegl:motion-blur-linear"
                                           "mblurlinear")))
      (gimp-drawable-filter-configure mblurlinear LAYER-MODE-REPLACE 1.0
                                      "length" len
                                      "angle" (clampTo180 angle))
      (gimp-drawable-merge-filter layer mblurlinear)
      (set! layer_oldx (car (gimp-drawable-get-offsets layer)))
      (set! layer_oldy (car (gimp-drawable-get-offsets layer)))
      (gimp-layer-resize layer layer_neww layer_newh (- layer_oldx layer_newx) (- layer_oldy layer_newy))
      (gimp-layer-set-offsets layer layer_newx layer_newy))))
...
(define (zoom_motion_blur layer stength cx cy)
  ...
  (let*  ((image (car (gimp-item-get-image layer)))
          (layer_w (car (gimp-drawable-get-width layer)))
          (layer_h (car (gimp-drawable-get-height layer)))
          (layer_x (car (gimp-drawable-get-offsets layer)))
          (layer_y (cadr (gimp-drawable-get-offsets layer)))
          (layer_neww layer_w)
          (layer_newh layer_h)
          (layer_newx layer_x)
          (layer_newy layer_y)
          (layer_oldx '())
          (layer_oldy '())
          (image_w (car (gimp-image-get-width image)))
          (image_h (car (gimp-image-get-height image)))
          (factor (/ stength 1000))
          (mblurzoom '()))
    (set! factor (min 1.0 factor))
    (set! factor (max -10.0 factor))
    (define (x_to_layerw_ratio k) (/ k layer_w) )
    (define (y_to_layerh_ratio k) (/ k layer_h) )
    (define (x_to_imagew_ratio k) (/ k image_w) )
    (define (y_to_imageh_ratio k) (/ k image_h) )

    (set! mblurzoom (car (gimp-drawable-filter-new layer "gegl:motion-blur-zoom" "mblurzoom")))
    (gimp-drawable-filter-configure mblurzoom LAYER-MODE-REPLACE 1.0
                                    "center-x" (x_to_layerw_ratio cx)
                                    "center-y" (y_to_layerh_ratio cy)
                                    "factor" factor)
    (gimp-drawable-merge-filter layer mblurzoom)
    (set! layer_oldx (car (gimp-drawable-get-offsets layer)))
    (set! layer_oldy (cadr (gimp-drawable-get-offsets layer)))
    (gimp-layer-resize layer layer_neww layer_newh (- layer_oldx layer_newx) (- layer_oldy layer_newy))
    (gimp-layer-set-offsets layer layer_newx layer_newy)))
   ...
   (let ((bp (descartes-to-blur-params dx dy)))
     (cons (lambda (layer target)
             (if (not (= dx dy 0))
                 (let* ((len (car bp))
                        (angle (cdr bp)))
                   (linear_motion_blur layer len angle) ; '() is returned
                   )))
           #f))))
...
(define (animstack-rb img params)
  (with-params
   ...
   (cons (lambda (layer target)
           (if (not (= angle 0))
               (let* ((layer_w (car (gimp-drawable-get-width layer)))
                      (layer_h (car (gimp-drawable-get-height layer)))
                      (layer_x (car (gimp-drawable-get-offsets layer)))
                      (layer_y (cadr (gimp-drawable-get-offsets layer)))
                      (layer_neww layer_w)
                 ...
                 (set! mblurcircular
                       (car (gimp-drawable-filter-new layer "gegl:motion-blur-circular" "mblurcircular")))
                 (gimp-drawable-filter-configure mblurcircular LAYER-MODE-REPLACE 1.0
                                                 "center-x" x
                                                 "center-y" y
                                                 "angle" (angle-constraint angle))
                 (gimp-drawable-merge-filter layer mblurcircular)
                 (set! layer_oldx (car (gimp-drawable-get-offsets layer)))
                 (set! layer_oldy (cadr (gimp-drawable-get-offsets layer)))
                 (gimp-layer-resize layer layer_neww layer_newh (- layer_oldx layer_newx) (- layer_oldy layer_newy))
                 (gimp-layer-set-offsets layer layer_newx layer_newy))))
...
(define (gaussian_blur layer rx ry)
  (let* ((layer_w (gimp-drawable-get-width layer))
         (layer_h (gimp-drawable-get-height layer))
         (layer_x (car (gimp-drawable-get-offsets layer)))
         (layer_y (cadr (gimp-drawable-get-offsets layer)))
         (layer_neww layer_w)
         (layer_newh layer_h)
         ...
         (layer_oldy '())
         (gaussblur '()))
    (set! gaussblur (car (gimp-drawable-filter-new layer
                                                   "gegl:gaussian-blur"
                                                   "gaussblur")))
    (gimp-drawable-filter-configure gaussblur LAYER-MODE-REPLACE 1.0
                                    "std-dev-x" (* 0.32 rx)
                                    "std-dev-y" (* 0.32 ry)
                                    "filter" "auto")
    (gimp-drawable-merge-filter layer gaussblur)
    (set! layer_oldx (car (gimp-drawable-get-offsets layer)))
    (set! layer_oldy (cadr (gimp-drawable-get-offsets layer)))
    (gimp-layer-resize layer layer_neww layer_newh (- layer_oldx layer_newx) (- layer_oldy layer_newy))
    (gimp-layer-set-offsets layer layer_newx layer_newy)))
   ...
   (let* ((kl (/ kl 30)) (kz (/ kz 100)))
     (cons (lambda (layer target)
             (let* ((motion (animstack-get-motion))
                    ...
                    (width (car (gimp-drawable-get-width layer)))
                    (height (car (gimp-drawable-get-height layer)))
                    (mb (animstack-mb img (list (* kl dx width) (* kl dy height))))
                    (zx (- (caddr motion)))
                    (zy (- (cadddr motion)))
...
(define (animstack-process-all-layers img)
  ...
  (let* ((layers (car (gimp-image-get-layers img))))
    (vector-for-each
     (lambda (layer)
       (gimp-progress-pulse)
       (animstack-process-layer img layer #f))
     layers))
  (gimp-context-pop)
  (gimp-image-thaw-layers img)
  (gimp-image-undo-group-end img)
...
(define (script-fu-animstack-process-all-filter InImage InDrawables)
  (animstack-process-all-layers InImage))

(script-fu-register-filter
 "script-fu-animstack-process-all-filter"
...
(script-fu-menu-register "script-fu-animstack-process-all-filter"
                         ;; FOR TRANSLATORS: Don't translate '<Image>/Filters/Animation/'
                         "<Image>/Filters/Animation/Timofei Shatrov")
(script-fu-register-i18n "script-fu-animstack-process-all-filter" "Standard")
    ...
    (vector-for-each
     (lambda (layer)
       (cond ((test layer) (fn layer))
             ((py-True? (car (gimp-item-is-group layer)))
...
(define (script-fu-reverse-mirror-layers-filter InImage InDrawables mode ignore-tagged)
  (let* ((img InImage) (drawable (vector-ref InDrawables 0)))
    (script-fu-reverse-mirror-layers img drawable mode ignore-tagged)))

(script-fu-register-filter
 "script-fu-reverse-mirror-layers-filter"
...
(script-fu-menu-register "script-fu-reverse-mirror-layers-filter"
                         _"<Image>/Image/Timofei Shatrov")
(script-fu-register-i18n "script-fu-reverse-mirror-layers-filter" "Standard")
